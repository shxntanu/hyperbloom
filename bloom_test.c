...
void TestBFSetBit() {
  ...
  int err = setBit(bf, 100);
  assert(err == 0, "setBit should not return an error");

  bool bitExists = getBit(bf, 100);
  assert(bitExists, "Bit 100 should be set");

  bool falseBit = getBit(bf, 1048575);
  assert(!falseBit, "Bit 1048575 should not be set");
